...
import com.onlinemall.order.utils.PayHelper;
import com.onlinemall.order.utils.PayState;
...
public class OrderController {
    ...
    public ResponseEntity<PageInfo<Order>> queryUserOrderList(
            @RequestParam(value = "page", defaultValue = "1") Integer page,
            @RequestParam(value = "rows", defaultValue = "5") Integer rows,
            @RequestParam(value = "status", defaultValue = "0") Integer status) {
        PageInfo<Order> result = this.orderService.queryUserOrderList(page, rows, status);
        if (result == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
    ...
    @GetMapping("list/all")
    public ResponseEntity<PageInfo<Order>> queryOrderListAll(
            @RequestParam(value = "page", defaultValue = "1") Integer page,
            @RequestParam(value = "rows", defaultValue = "5") Integer rows,
            @RequestParam(value = "search",defaultValue = "")String keyword) {
        PageInfo<Order> pageInfo = this.orderService.queryOrderListAll(page,rows,keyword);
        if (pageInfo == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
